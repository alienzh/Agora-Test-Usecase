...
int join_rtc_and_receive(SampleOptions options, agora::base::IAgoraService *service)
{
  ...
  // if(!options.local_vos_ip.empty()){
  //   p->setParameters("{\"rtc.force_local\":true}");
  // //  p->setString("rtc.local_ap_list",options.local_vos_ip.c_str());
  //  int a =  p->setString("rtc.vos_list",options.local_vos_ip.c_str());
  //  p->setParameters("{\"rtc.vos_list\":[\"10.180.0.140:4701\"]}");
  //  printf("it is %d \n",a);
  // }
...
int main(int argc, char *argv[])
{
  ...
  optParser.add_long_opt("rtmpUrl", &options.rtmpUrl,
						   "The rtmp cdn url");
    optParser.add_long_opt("localvos", &options.local_vos_ip,
						   "The local_vos_ip  ");
